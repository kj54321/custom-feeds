...
proto_openconnect_init_config() {
	...
	proto_config_add_int "globalprotect"
...
proto_openconnect_setup() {
	...
	json_get_vars server port interface username serverhash authgroup password password2 token_mode token_secret token_script os csd_wrapper mtu juniper globalprotect
	...
	if [ "${juniper:-0}" -gt 0 ]; then
		append cmdline "--protocol=nc"
	fi

	if [ "${globalprotect:-0}" -gt 0 ]; then
